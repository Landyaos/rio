...
class MyProxyMiddleware(object):
    ...
    def __init__(self):
        ...
        self.PROXY_POOL = self._proxy_pool_get(5)
    ...
    def _proxy_pool_get(self, num):
        ...
        for each in range(num):
            res = self.http.request('GET', settings.PROXY_API.format(1))
            data = json.loads(res.data.decode())
            if data[settings.PROXY_LEFT] < 1:
                break
            else:
                ans.append(data['domain'] + ':' + str(data['port'][0]))
    ...
    def process_request(self, request, spider):
        if len(self.PROXY_POOL) == 0:
            try:
                self.PROXY_POOL.extend(self._proxy_pool_get(5))
            except Exception as e:
                logging.error(e)
                spider.crawler.engine.close_spider(spider, "代理ip消耗殆尽，关闭spider")

        request.meta['proxy'] = 'https://' + random.choice(self.PROXY_POOL)
...
# num = 2
# for each in range(num):
#     res = http.request('GET', settings.PROXY_API.format(1))
#     data = json.loads(res.data.decode())
#     print(data)
#
#     if data[settings.PROXY_LEFT] < 1:
